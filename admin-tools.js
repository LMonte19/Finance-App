...
function ensureStyles() {
  ...
  style.textContent = `
    ...
    .admin-profile-card select { box-sizing:border-box; max-width:100%; }
...
function ensureAdminMenuItems() {
  ...
  insertMenuButton({
    ...
    onOpen: () => refreshProfilesPage({ force: true }),
  });
  ...
  insertMenuButton({
    ...
    onOpen: () => refreshReportsPage({ force: true }),
  });
  ...
  insertMenuButton({
    ...
    label: "Maintenance",
    beforeDefaults: false,
    onOpen: () => refreshMaintenancePage({ force: true }),
  });
}
...
async function refreshMaintenancePage({ force = false } = {}) {
  ...
  content.innerHTML = `
    <div class="card">
      ...
      <div class="muted">Admin-only maintenance tools.</div>
    </div>
    <div class="card">
...
async function refreshVisibleAdminTools() {
  try {
    ...
    if (activePage("profilesPage") && qs("profilesPageContent")?.dataset.loaded !== "true") {
      await refreshProfilesPage();
    }

    if (activePage("reportsPage") && qs("reportsPageContent")?.dataset.loaded !== "true") {
      await refreshReportsPage();
    }

    if (activePage("maintenancePage") && qs("maintenancePageContent")?.dataset.loaded !== "true") {
